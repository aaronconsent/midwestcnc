...
.insight-pillar-card h3 { margin: var(--s-2) 0; font-size: 1.18rem; }
.insight-pillar-card p { color: var(--muted); margin: var(--s-1) 0 var(--s-3) 0; font-size: 0.95rem; flex-grow: 1; }
...
.insight-card h3 { margin: var(--s-1) 0; font-size: 1.08rem; line-height: 1.3; }
.insight-card p { color: var(--muted); margin: var(--s-2) 0 var(--s-3) 0; font-size: 0.92rem; flex-grow: 1; }
...
/* Article body — Key Takeaways block is just an H2 + UL in the markdown,
   so we style by H2 text content via a sibling selector. Use a class on
   the H2 instead (added by the generator below). Simpler approach: style
   the first <ul> after an h2 whose text starts with "Key Takeaways" via
   a class the markdown converter doesn't emit — so we add CSS for a
   .insight-takeaways class that the markdown generator can use. Even
   without that class, the structural rules below keep articles tidy. */
.layout-default main article h2:first-of-type + ul,
.insight-takeaways {
...
.layout-default main article h2:first-of-type + ul li,
...
/* Definitional opener — first paragraph under each H2 gets a slightly
   heavier weight to reinforce the "quote me" cue. Subtle; not screaming. */
.layout-default main article h2 + p {
  font-size: 1.05rem;
  line-height: 1.65;
}
...
.insight-sources h2 { font-size: 1.05rem; color: var(--muted); }
.insight-sources li { font-size: 0.92rem; color: var(--muted); }
