...
.home-hero {
  display: grid;
  /* Anchor the LEFT gutter to match the rest of the page's content
     alignment. The .section-inner.wide container that wraps every other
     section is max-width 1140px (--max-wide) centered. So the left
     gutter here computes the same offset: (viewport - max-wide) / 2,
     with a min of 1.25rem so it never collapses to zero on tablets.
     The image column then takes EVERYTHING REMAINING after text+gap
...
@media (max-width: 600px) {
  .home-hero-text > h1 { font-size: clamp(1.55rem, 7vw, 2.1rem); }
  .home-hero-image img { aspect-ratio: 3 / 2; max-height: 240px; }
}
